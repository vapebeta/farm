...
void Visuals::ManualAntiAim() {
	...
	if (g_hvh.m_left) {
		render::arrows.wstring(x - 71, y - 31, color, L"\u2bc7");
	}
	
	if (g_hvh.m_right) {
		render::arrows.wstring(x + 44, y - 31, color, L"\u2bc8");
	}
	
	if (g_hvh.m_back) {
		render::arrows.wstring(x - 14, y + 24, color, L"\u2bc6");
	}
...
void Visuals::DrawPlayer(Player* player) {
	...
	if (g_menu.main.players.history_skeleton.get()) {
	...
	// draw flags.
	{
		...
		//std::string resolve_name{ g_resolver.resolver_state[player->index()] }; // resolve mode
		...
		for (auto it = items.begin(); it != items.end(); ++it) {
			...
			// resolver
			//if (*it == 7 && g_cl.m_processing)
			//	if (dormant)
			//		flags.push_back({ resolve_name, { 130,130,130, low_alpha } });
			//	else
			//		flags.push_back({ resolve_name, { 255,255,255, low_alpha} });
...
void Visuals::DrawHistorySkeleton(Player* player, int opacity) {
	...
	for (int i{}; i < hdr->m_num_bones; ++i) {
		...
		get_bone(record->m_bones, bone_pos, i);
		get_bone(record->m_bones, parent_pos, parent);

		Color clr = g_menu.main.players.history_skeleton_col.get();
		clr.a() = g_menu.main.players.history_skeleton_alpha.get();
