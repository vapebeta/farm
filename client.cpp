...
ulong_t __stdcall Client::init(void* arg) {
	...
	g_cl.m_user = XOR("farmhack");

	// stop here if we failed to acquire all the data needed from csgo.
	if (!g_csgo.init())
		return 0;

	g_csgo.m_engine->ExecuteClientCmd((XOR("fps_max 750")));

	// welcome the user.
	g_notify.add(tfm::format(XOR("welcome to %s, nigga\n"), g_cl.m_user));
...
void Client::DrawHUD() {
	...
	auto full = tfm::format("farmhack %sms", ms);
	...
	render::round_rect(w - 15 - full_size, 15, full_size, 19, 2, colors::dark);
	render::gradient1337(w - 15 - full_size + 2, 17, start_size, 15, Color(color.r(), color.g(), color.b(), 55), Color(0, 0, 0, 0));
	render::esp.string(w - 15 - full_size + 5, 17, color, start);
	render::esp.string(w - 15 - full_size + start_size, 17, Color(255, 255, 255, 255), end);
...
void Client::ClanTag()
{
	...
	auto SetClanTag = [&](std::string tag) -> void {
		...
		SetClanTagFn(tag.c_str(), XOR("farmhack"));
	};

	std::string szClanTag = XOR("farmhack");
	...
	if (g_menu.main.misc.clantag.get()) {
		...
		if (iPrevFrame != int(g_csgo.m_globals->m_curtime * 3.6) % 19) {
			...
			case 2: {  SetClanTag("fa"); break; }
			case 3: {  SetClanTag("far"); break; }
			case 4: {  SetClanTag("farm"); break; }
			case 5: {  SetClanTag("farmh"); break; }
			case 6: {  SetClanTag("farmha"); break; }
			case 7: {  SetClanTag("farmhac"); break; }
			case 8: {  SetClanTag("farmhack"); break; }
			case 9: {  SetClanTag("farmhack"); break; }
			case 10: { SetClanTag("farmhack"); break; }
			case 11: { SetClanTag("farmhac"); break; }
			case 12: { SetClanTag("farmha"); break; }
			case 13: { SetClanTag("farmh"); break; }
			case 14: { SetClanTag("farm"); break; }
			case 15: { SetClanTag("far"); break; }
			case 16: { SetClanTag("fa"); break; }
			case 17: { SetClanTag("f"); break; }
...
void Client::DoMove(CUserCmd* cmd) {
...
void Client::print(const std::string text, ...) {
	...
	// print to console.
	g_csgo.m_cvar->ConsoleColorPrintf(g_gui.m_color, XOR("[farmhack] "));
