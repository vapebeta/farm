...
/*
bool penetration::TraceToExitShort( vec3_t& point, vec3_t& dir, const float step_size, float max_distance ) {
	float distance = 0;
	...
	while( distance <= max_distance ) {
		...
		point += dir * distance;
		const auto point_contents = g_csgo.m_engine_trace->GetPointContents( point, MASK_SHOT_HULL );
...
float penetration::get_thickness( vec3_t& start, vec3_t& end, const float distance ) {
	auto dir = end - start;
	auto step = start;
	...
	while( true ) {
		g_csgo.m_engine_trace->TraceRay( Ray( step, end ), MASK_SHOT_HULL, &filter, &trace );

		if( !trace.hit( ) )
			return thickness;

		const auto last_step = trace.m_endpos;
		step = trace.m_endpos;

		if( ( end - start ).length( ) <= ( step - start ).length( ) )
			break;

		if( !TraceToExitShort( step, dir, 5, 150.f ) )
			return FLT_MAX;
